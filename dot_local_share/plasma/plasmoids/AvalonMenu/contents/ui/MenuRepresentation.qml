...
PlasmaCore.Dialog {
    ...
    function popupPosition(width, height) {
        ...
        if (plasmoid.location === PlasmaCore.Types.BottomEdge) {
            ...
            y = screen.height - height;// - panelSvg.margins.top;
        } else if (plasmoid.location === PlasmaCore.Types.TopEdge) {
            horizMidPoint = screen.x + (screen.width / 2);
            var appletBottomLeft = parent.mapToGlobal(0, parent.height);
            x = (appletBottomLeft.x < horizMidPoint) ? screen.x: (screen.x + screen.width) - width;
            y = parent.height;// + panelSvg.margins.bottom;
        } else if (plasmoid.location === PlasmaCore.Types.LeftEdge) {
            vertMidPoint = screen.y + (screen.height / 2);
            appletTopLeft = parent.mapToGlobal(0, 0);
            x = parent.width;// + panelSvg.margins.right;
            y = (appletTopLeft.y < vertMidPoint) ? screen.y: (screen.y + screen.height) - height;
        } else if (plasmoid.location === PlasmaCore.Types.RightEdge) {
            vertMidPoint = screen.y + (screen.height / 2);
            appletTopLeft = parent.mapToGlobal(0, 0);
            x = appletTopLeft.x;// - panelSvg.margins.left - width;
            y = (appletTopLeft.y < vertMidPoint) ? screen.y : (screen.y + screen.height) - height;
        }
